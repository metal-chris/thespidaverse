...
export function WebGraphTree({ nodes, edges }: WebGraphTreeProps) {
  ...
  const renderCustomNode = useCallback(
    ({ nodeDatum, toggleNode }: CustomNodeElementProps) => {
      ...
      // More accurate text width calculation for uniform badge sizing
      const textLength = nodeDatum.name.length;
      
      // Character width varies by font weight - use more precise multipliers
      let avgCharWidth: number;
      if (fontWeight >= 700) {
        avgCharWidth = fontSize * 0.58; // Bold text is wider
      } else if (fontWeight >= 600) {
        avgCharWidth = fontSize * 0.56; // Semi-bold
      } else {
        avgCharWidth = fontSize * 0.54; // Regular
      }
      
      const textWidth = textLength * avgCharWidth;
      
      // Uniform padding for consistent badge appearance
